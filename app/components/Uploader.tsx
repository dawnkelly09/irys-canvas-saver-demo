	...
	const handleUpload = async () => {
		...
		if (files.length > 1) {
			try {
				...
				let [manifestId, receiptId] = await fundAndUpload(filesToUpload, []);
		...
		<div className={`bg-white rounded-lg border shadow-2xl mx-auto min-w-full`}>
			<div className="flex p-5">
				<div className={`space-y-6 ${memoizedPreviewURL && memoizedReceiptView ? "w-1/2" : "w-full"}`}>
					<div
						className="border-2 border-grey rounded-lg p-4 text-center"
					...
					>
						<p className="text-black mb-2 ">Drag and drop files here</p>
						<input type="file" multiple onChange={handleFileUpload} className="hidden" />
						<button
							onClick={resetFilesAndOpenFileDialog}
							className={`w-full min-w-full py-2 px-4 bg-[#DBDEE9] text-black font-bold rounded-md flex items-center justify-center transition-colors duration-500 ease-in-out  ${
						...
						<div className="flex flex-col space-y-2">
							{files.map((file, index) => (
								<div key={index} className="flex items-center justify-start mb-2">
									<span className="mr-2 text-black">{file.file.name}</span>
									{file.isUploaded && (
										<>
